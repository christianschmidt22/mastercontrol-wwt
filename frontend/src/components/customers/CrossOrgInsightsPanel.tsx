/**
 * CrossOrgInsightsPanel.tsx
 *
 * "Mentioned by other orgs" section rendered above the tile grid on the
 * customer detail page. Surfaces agent_insight notes from OTHER orgs' threads
 * that target this customer — e.g. when a Cisco OEM agent mentions Fairview.
 *
...
export interface CrossOrgInsightsPanelProps {
  orgId: number;
...
export function CrossOrgInsightsPanel({
  orgId,
  _useCrossOrgInsights,
  _useConfirmInsight,
  _useRejectInsight,
}: CrossOrgInsightsPanelProps) {
  ...
  // Don't render the section at all when loading and no data yet — avoids
  // layout shift. Panel collapses when empty.
  if (isLoading && insights.length === 0) return null;
  if (!isLoading && insights.length === 0) return null;

  return (
    <section
      aria-label="Cross-org insights"
    ...
    >
      ...
      >
        Mentioned by other orgs
        <span
          aria-label={`${insights.length} cross-org insight${insights.length === 1 ? '' : 's'}`}
          style={{
            marginLeft: 8,
            fontSize: 12,
            fontWeight: 400,
      ...
      <ul
        role="list"
        aria-label="Cross-org insights list"
        style={{
          listStyle: 'none',
          margin: 0,
          padding: 0,
