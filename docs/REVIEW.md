...
- **Q-1 (vermilion budget)**: Confirm R-008's "one zone at rest + enumerated transients" rewording. The alternative is to demote overdue and active-status pills entirely, which loses signal. Default plan: rewording.
- **Q-2 (drag-reorder vs. ↑/↓ buttons)**: Drag-and-drop tile reordering meaningfully improves UX over ↑/↓ buttons but adds `@dnd-kit` (~12 KB). Default plan: ship drag with R-009 keyboard model. Confirm or downgrade to buttons.
- **Q-3 (Phase 2 scheduler architecture)**: Performance #10 says collapse to Task Scheduler only (no Windows Service). Security #5a says if Service stays, run as interactive user not LocalSystem. Pick one before Phase 2 starts. Default lean: Task Scheduler only.
- **Q-4 (Insights queue UX)**: R-002 introduces unconfirmed `agent_insight` notes. Where does the user accept/reject — inline on the org's notes feed, on the Agents page Insights tab, or both? Default plan: both; the Insights tab is the bulk-review surface.
- **Q-5 (`record_insight` allowlist scope)**: Should the allowlist include orgs mentioned anywhere in the current thread, or only the latest user message + the current org? Stricter is safer; looser is more useful. Default plan: latest user message ∪ current org ∪ current org's `note_mentions` rows.
